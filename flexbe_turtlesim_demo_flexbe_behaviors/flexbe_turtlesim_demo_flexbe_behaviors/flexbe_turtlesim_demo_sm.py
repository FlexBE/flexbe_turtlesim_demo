...
Demonstrates several FlexBE capabilities including synchronous and async
service calls, action interfaces, and operator input.
    ...
    def create(self):
        ...
        with _sm_goto_0:
            ...
            OperatableStateMachine.add('GetPose',
                                       InputState(request=3,
                                                  message='Input pose (x, y, radians)',
                                                  timeout=1.0,
                                                  action_topic='flexbe/behavior_input'),
                                       transitions={'received': 'GoToPose',
                                                    'aborted': 'GoToFail',
                                                    'no_connection': 'GoToFail',
                                                    'data_error': 'GoToFail'},
                                       autonomy={'received': Autonomy.Off,
                                                 'aborted': Autonomy.Off,
                                                 'no_connection': Autonomy.Off,
                                                 'data_error': Autonomy.Off},
                                       remapping={'data': 'pose'})

            # x:585 y:258
            OperatableStateMachine.add('GoToFail',
                                       LogState(text='Failed to go to pose',
            ...
            OperatableStateMachine.add('GoToPose',
                                       TeleportAbsoluteState(turtle_name='turtle1',
                                                             x=0.0,
                                                             y=0.0,
                                                             theta=0.0,
                                                             call_timeout=3.0,
                                       ...
                                       transitions={'done': 'finished',
                                                    'failed': 'GoToFail',
                                                    'call_timeout': 'GoToFail',
                                                    'unavailable': 'GoToFail'},
                                       autonomy={'done': Autonomy.Off,
                                                 'failed': Autonomy.Off,
                                                 'call_timeout': Autonomy.Off,
        ...
        with _sm_eightmove_1:
            ...
            OperatableStateMachine.add('Forward0',
                                       TimedCmdVelState(target_time=3.99,
                                                        velocity=0.5,
                                                        rotation_rate=0.0,
                                                        cmd_topic=cmd_vel,
                                                        desired_rate=50),
            ...
            OperatableStateMachine.add('Forward1',
                                       TimedCmdVelState(target_time=7.99,
                                                        velocity=0.5,
                                                        rotation_rate=0.0,
                                                        cmd_topic=cmd_vel,
                                                        desired_rate=50),
            ...
            OperatableStateMachine.add('LeftTurn',
                                       TimedCmdVelState(target_time=5.77,
                                                        velocity=0.5,
                                                        rotation_rate=0.667,
                                                        cmd_topic=cmd_vel,
                                                        desired_rate=50),
        ...
        with _state_machine:
            ...
            OperatableStateMachine.add('ClearEntry',
                                       ClearTurtlesimState(service_name='/clear',
                                                           wait_timeout=3.0),
            ...
            OperatableStateMachine.add('ClearFailed',
                                       LogState(text='Failed to clear Turtlesim window!',
                                                severity=Logger.REPORT_HINT),
                                       transitions={'done': 'Operator'},
                                       autonomy={'done': Autonomy.Off})
            ...
            OperatableStateMachine.add('ClearLog',
                                       LogState(text='Clear turtlesim window ...',
                                                severity=Logger.REPORT_HINT),
            ...
            OperatableStateMachine.add('FailedEight',
                                       LogState(text='Failed Eight pattern',
                                                severity=Logger.REPORT_HINT),
                                       transitions={'done': 'Operator'},
                                       autonomy={'done': Autonomy.Off})
            ...
            OperatableStateMachine.add('Finished Eight-pattern',
                                       LogState(text='Finished 8 pattern',
                                                severity=Logger.REPORT_HINT),
                                       transitions={'done': 'Operator'},
                                       autonomy={'done': Autonomy.Off})
            ...
            OperatableStateMachine.add('GoHome',
                                       LogState(text='Go to home position',
                                                severity=Logger.REPORT_HINT),
                                       transitions={'done': 'Home'},
                                       autonomy={'done': Autonomy.Off})
            ...
            OperatableStateMachine.add('Operator',
                                       OperatorDecisionState(outcomes=['Home',
                                                                       'Eight',
                                                                       'Quit',
                                                                       'Clear',
                                                                       'Rotate',
                                                                       'Pose'],
                                                             hint="Suggest 'Eight' for pattern move",
                                                             suggestion='Eight'),
                                       transitions={'Home': 'GoHome',
                                                    'Eight': 'EightMove',
                                                    'Quit': 'finished',
                                                    'Clear': 'ClearLog',
                                                    'Rotate': 'Turtlesim Input State Behavior',
                                                    'Pose': 'GoTo'},
                                       autonomy={'Home': Autonomy.Full,
                                                 'Eight': Autonomy.High,
                                                 'Quit': Autonomy.Full,
                                                 'Clear': Autonomy.Full,
                                                 'Rotate': Autonomy.Full,
                                                 'Pose': Autonomy.Full})

            # x:934 y:534
            OperatableStateMachine.add('PoseDone',
                                       LogState(text='Updated pose',
                                                severity=Logger.REPORT_HINT),
                                       transitions={'done': 'Operator'},
                                       autonomy={'done': Autonomy.Off})
            ...
            OperatableStateMachine.add('RotateLog',
                                       LogState(text='Rotation complete',
                                                severity=Logger.REPORT_HINT),
                                       transitions={'done': 'Operator'},
                                       autonomy={'done': Autonomy.Off})
            ...
            OperatableStateMachine.add('StopCmd',
                                       TimedCmdVelState(target_time=0.01,
                                                        velocity=0.0,
                                                        rotation_rate=0.0,
                                                        cmd_topic=cmd_vel,
                                                        desired_rate=50),
            ...
            OperatableStateMachine.add('Turtlesim Input State Behavior',
                                       ...
                                       transitions={'finished': 'RotateLog',
                                                    'failed': 'RotateFailed'},
                                       autonomy={'finished': Autonomy.Inherit,
                                                 'failed': Autonomy.Inherit})
